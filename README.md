...
3. **Bestandsdaten:** Wenn vorhanden, `import-legacy.sql` einmalig VOR der Einrichtung eines Adminprofils ausführen. Identische Mitarbeiterdubletten werden anhand der E-Mail-Adresse einmal übernommen. Die Übernahme bricht bei widersprüchlichen Dubletten, fehlenden Daten, mehrdeutigen Namen, ungültigen Zeiträumen oder überlappenden gleichartigen Buchungen vollständig ab. Die alte Tabelle `department` wird als Urlaubstabelle behandelt. Alte Passwörter werden nicht übernommen. Alle importierten Personen starten als Mitarbeiter; Rollen anschließend ausdrücklich prüfen und vergeben. Bei einem Fehler Altbestand zuerst prüfen, nicht ungeprüft Daten löschen.
